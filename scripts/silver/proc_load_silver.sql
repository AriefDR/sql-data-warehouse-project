...
BEGIN
    ...
    SELECT 
        prd_id,
        REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- extract category from prd_key and replace '-' with '_' to match with dimensi kategori produk di ERP
        SUBSTRING(prd_key, 7, LENGTH(prd_key)) AS prd_key, -- extract product code from prd_key
        prd_nm,
    ...
    SELECT
        ...
        CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
            ...
            ELSE TRIM(cntry)
        END AS cntry -- Normalize and Handle missing or blank country coded
